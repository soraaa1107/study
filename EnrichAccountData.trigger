trigger EnrichAccountData on Account (after insert, after update) {
    ...
    // API連携処理を実行（Bulkifyを考慮してリストで渡す）
    if (!accountsToProcess.isEmpty()) {
        AccountDataEnrichmentService.enrichAccountData(accountsToProcess);
    }
}